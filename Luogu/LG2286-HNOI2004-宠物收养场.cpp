...
int main() {
	...
	REP(i,1,n) {
		int op=read(),a=read();
		if(s[op^1].size[s[op^1].rt]) {
			if(mp[op^1][a]) mp[op^1][a]--,s[op^1].del(a);
			else {
				s[op^1].insert(a);
				int pre=s[op^1].val[s[op^1].pre()],nxt=s[op^1].val[s[op^1].nxt()];
				if(pre==0) pre=nxt;
				if(nxt==0) nxt=pre;
				if(pre==0) {
					s[op].insert(a),mp[op][a]++;
					continue;;
				}
				s[op^1].del(a);
				if(abs(pre-a)<=abs(nxt-a)) {
					s[op^1].del(pre);
					mp[op^1][pre]--;
					ans=(ans+abs(pre-a))%MOD;
				}
				else {
					mp[op^1][nxt]--;
					s[op^1].del(nxt);
					ans=(ans+abs(nxt-a))%MOD;
				}
			}
		}	
		else s[op].insert(a),mp[op][a]++;

...
int ask(int l,int r,int c) {
	int ans=0;
	REP(i,l,min(r,pos[l]*m)) if(s[i]+tag[pos[l]]<c) ans++;
	if(pos[l]!=pos[r]) REP(i,(pos[r]-1)*m+1,r) if(s[i]+tag[pos[r]]<c) ans++;
	REP(i,pos[l]+1,pos[r]-1) ans+=lower_bound(g[i].begin(),g[i].end(),c-tag[i])-g[i].begin();
	return ans;
}
